...
const OrdersView = () => {
    ...
    // --- CREATE ORDER ---
    // --- OrdersView.jsx mein handleCreate update karein ---
    const handleCreate = async (e) => {
        ...
        try {
            ...
            // 1. Pickup Location: Admin ka warehouse/shop location
            const adminPickupLat = 19.2307; // Borivali Latitude (Replace with your shop location)
            const adminPickupLng = 72.8567; // Borivali Longitude

            // 2. Drop Location: Order address se dhoondo, nahi toh 0 (Manual selection required)
            const dropLat = coords ? coords.lat : 0;
            const dropLng = coords ? coords.lng : 0;

            if (!coords) {
                alert("⚠️ Map could not find the exact Drop Address. Driver will need to set it manually from their app.");
            }

            const payload = {
                ...form,
                adminEmail: user.email,
                pickupLat: adminPickupLat,
                pickupLng: adminPickupLng,
                dropLat: dropLat,
                dropLng: dropLng
            };
